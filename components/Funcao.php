<?php
	...
	class Funcao{
		...
		public static function prepararLink($string){
			...
			$string = self::substituiCaracteres($string);

			$string = empty($string) ? "expotec" : preg_replace("![^a-z0-9]+!i", "-", strtolower($string));
			return $string;
		}

		public static function desprepararLink($string){
		...
		public static function redimensionarImagem( $file, $largura = '', $altura = '' ) {
			...
			$imagem->resize($largura, $altura, 'inside', 'down')->saveToFile($file);

		}

		public static function gerarNomeImagem( $imagem, $nome ) {
			sleep(2);
			return strtolower(Funcao::prepararLink($nome)).'-'.md5(date('YmdHis')).'.'.pathinfo($imagem, PATHINFO_EXTENSION);

		}

		public static function enviarEmail($para, $assunto, $corpo, $nome='') {

			require_once "Mail.php";

			$headers = array ('From' => $nome."<noreply@expotec.org.br>",
					'To' => $para,
					'Subject' => $assunto,
					'Content-Type' => "text/html; charset=UTF-8");

			//$smtp = Mail::factory('smtp',
				//	array ('host' => 'mail.nossaamazonia.org.br',
					//	'port' => 587,
					//	'auth' => true,
					//	'username' => 'contato@nossaamazonia.org.br',
					//	'password' => 'ContatoNA2013'));

      $smtp = Mail::factory('smtp',
					array ('host' => 'mail.expotec.org.br',
							'port' => 587,
							'auth' => true,
							'username' => 'noreply@expotec.org.br',
							'password' => 'Mud@rExpo'));
			...
			if (PEAR::isError($mail)) {
				print "<script>alert('Ocorreu um erro no envio do email, tente novamente mais tarde.');</script>";
			} else {
				return $mail;
			}
		...
		public static function apenasNumeroFone($numero,$codPais=55){
		
			if($numero != '(00) 0000-0000' && !empty($numero)){
			  $numero = str_replace('(','',$numero);		
			  $numero = str_replace(')','',$numero);		
			  $numero = str_replace(' ','',$numero);		
			  $numero = str_replace('-','',$numero);		
				return $codPais.$numero;
			}
		...
		public static function resumo($texto, $qtd=3)
		{
			$texto = strip_tags($texto);
			$resumo1 = explode(" ",$texto);
			for ($x=0;$x<$qtd;$x++) {
				$resumo .= $resumo1[$x].' ';
			}
			if($qtd<=count($resumo1)) {
				$resumo .= '...';
			}
		...
		public static function tratarYoutubeUrlEmbed($youtubeUrl) {
        ...
        public static function tratarYoutubeUrlVideoThumb($youtubeUrl) {
            $retorno = array();
            $url = parse_url($youtubeUrl);
			if( strpos($url['host'], "youtu.be") !== false ) {
				$id = substr($url['path'], 1);
            ...
            $retorno['video'] = 'https://www.youtube.com/watch?v='.$id;
            $retorno['thumbnail'] = 'http://img.youtube.com/vi/'.$id.'/hqdefault.jpg';
